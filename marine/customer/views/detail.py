...
class CustomerDetailView(AdminStaffRequiredMixin, DetailView):
    model = Customer
    template_name = "marine/customer_detail.html"
    context_object_name = "customer"

    def get_queryset(self):
        return Customer.objects.filter(id=self.kwargs["pk"]).values(
            "name",
            "contact",
            "address",
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["customer_object"] = Customer.objects.get(id=self.kwargs["pk"])
        return context
